...
SELECT ?concept ?description WHERE { 
  ?s a owl:Class ; rdfs:label ?concept .
...
~~~~
SELECT ?concept ?description WHERE { 
  ?s a owl:ObjectProperty ; rdfs:label ?concept . 
  optional { ?s dc:description ?description .  }
} 
~~~~
